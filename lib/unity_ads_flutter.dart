...
class UnityAdsFlutter {
  ...
  static Future<Null> prepareBanner(final String placementId) async{
    await _channel.invokeMethod('loadBanner',{'placementId':placementId,'listener':_listener});
  }
  static Future<Null> loadBanner(final String placementId) async{
    await _channel.invokeMethod('loadBanner',{'placementId':placementId});
  }
  static Future<Null> destroy() async{
...
abstract class UnityAdsListener {
  Future<Null> _handle(MethodCall methodCall) async {
    ...
    else if(methodCall.method=='onUnityBannerLoaded')
      onUnityBannerLoaded(methodCall.arguments['placementId'],methodCall.arguments['view']);
    else if(methodCall.method=='onUnityBannerUnloaded')
      onUnityBannerUnloaded(methodCall.arguments['placementId']);
    else if(methodCall.method=='onUnityBannerShow')
      onUnityBannerShow(methodCall.arguments['placementId']);
    else if(methodCall.method=='onUnityBannerClick')
      onUnityBannerClick(methodCall.arguments['placementId']);
    else if(methodCall.method=='onUnityBannerHide')
      onUnityBannerHide(methodCall.arguments['placementId']);
    else if(methodCall.method=='onUnityBannerError')
      onUnityBannerError(methodCall.arguments['message']);
  }
